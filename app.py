...
# แกะดีไซน์หน้าเว็บเดิมของคุณมาทำระบบ Grid + Grouping แบบ 100%
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <style>
        body { background-color: #121212; color: #e0e0e0; font-family: sans-serif; padding-bottom: 50px; }
        .main-title { text-align: center; color: #ffca28; font-weight: bold; margin-top: 20px; margin-bottom: 25px; }
        .card-custom { background-color: #1e1e1e; border: 1px solid #333; border-radius: 8px; color: #fff; }
        .section-title { font-size: 1.2rem; font-weight: bold; margin-top: 30px; margin-bottom: 15px; }
        
        /* สไตล์กล่องกลุ่มบอสหลัก (เบอร์บอสเด่น) */
        .boss-group-wrapper { background-color: #161616; border: 1px solid #2d2d2d; border-radius: 8px; padding: 15px; margin-bottom: 15px; }
        
        /* ดีไซน์การ์ดย่อย Ch. ข้างในแบบที่คุณชอบ */
...
<body class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <a href="/reset_db" onclick="return confirm('ต้องการรีเซ็ตฐานข้อมูล ล้างข้อมูลบอสทั้งหมดใช่ไหม?')" class="btn btn-sm btn-outline-danger">⚙️ รีเซ็ตระบบ</a>
    </div>

    <div class="card-custom p-3 mb-4">
        <h5 class="text-warning mb-3">➕ บันทึกบอสใหม่</h5>
        <form action="/add" method="POST" class="row g-2">
            <div class="col-4"><input type="text" name="boss_id" class="form-control form-input" placeholder="เลขบอส เช่น 123" required></div>
            ...
            <div class="col-12 mt-3"><button type="submit" class="btn btn-warning w-100 fw-bold">บันทึกข้อมูล</button></div>
        </form>
    </div>
        ...
        <div class="col">
            <div class="boss-group-wrapper" style="border-top: 3px solid #ff4757;">
                ...
                <div class="row row-cols-1 row-cols-md-2 g-2">
                    ...
                    <div class="col">
                        <div class="sub-card_inphase sub-card-inphase d-flex justify-content-between align-items-center">
                            <div>
...
def index():
    ...
    try:
        ...
        # --- จัดกลุ่มบอสอินเฟส บอสเบอร์เดียวกันจะยุบรวมเข้ากล่องเดียวกัน ---
        in_phase_dict = {}
        ...
        # --- จัดกลุ่มบอสรอเกิด บอสเบอร์เดียวกันจะยุบรวมเข้ากล่องเดียวกัน ---
        upcoming_dict = {}
        ...
        print(f"⚠️ เกิดข้อผิดพลาดฝั่ง Python: {e}")
