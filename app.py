...
# HTML แบบอัปเกรดระบบส่งข้อมูลหลังบ้าน ไม่เปลี่ยนหน้าเว็บ = ไม่มีวันหน้าขาว
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="th">
<head>
...
<body class="container">
    ...
    <div class="card-custom p-3 mb-4">
        ...
        <form id="addBossForm" onsubmit="submitAddForm(event)" class="row g-2">
            <div class="col-4"><input type="text" id="boss_id" class="form-control form-input" placeholder="เลขบอส เช่น 95" required></div>
            <div class="col-4"><input type="number" id="ch" class="form-control form-input" placeholder="Ch. เช่น 1" required></div>
            <div class="col-4"><input type="text" id="time_input" class="form-control form-input" placeholder="นาที เช่น -5"></div>
                    ...
                    <div class="col">
                        <div class="sub-card-inphase d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex gap-1">
                                ...
                                <button onclick="runApi('/delete/{{ group.boss_id }}/{{ item.ch }}')" class="btn btn-sm btn-dark text-danger border-secondary">🗑️</button>
                            </div>
                        </div>
                    </div>
                    {% endfor %}
    ...
    <script>
        ...
        // ฟังก์ชันหัวใจหลัก: ยิงข้อมูลแบบ Fetch API ยิงเงียบ ๆ หลังบ้าน หน้าเว็บไม่โหลดใหม่ = ไม่ขาวแน่นอน
        function runApi(url) {
            fetch(url)
